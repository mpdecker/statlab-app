...
export function clrTransform(data, vars) {
  ...
  const result = X.map(row => {
    ...
    return row.map(v => +(Math.log(Math.max(v, 1e-10)) - Math.log(gm)).toFixed(6));
  });
  const n = data.length;
...
export function ilrTransform(data, vars) {
  if (!data || data.length < 5 || !vars || vars.length < 2) return null;
  const clr = clrTransform(data, vars);
  if (!clr) return null;
  const p = vars.length;
  const Psi = Array.from({ length: p - 1 }, (_, i) => {
    const row = Array(p).fill(0);
    const k = i + 1;
    const sqrtK = Math.sqrt(k * (k + 1));
    for (let j = 0; j <= i; j++) row[j] = 1 / sqrtK;
    row[i + 1] = -k / sqrtK;
    return row;
  });
  const result = clr.transformed.map(row => Psi.map(psi => +psi.reduce((s, v, j) => s + v * row[j], 0).toFixed(6)));
...
export function compRegression(data, yVar, compVars, xVars = []) {
  ...
  const ilr = ilrTransform(data, compVars);
  if (!ilr) return null;
  const y = data.map(r => +r[yVar]);
  const Xraw = ilr.transformed.map((row, i) => [...row, ...xVars.map(v => +data[i][v])]);
  const n = Xraw.length;
  ...
  const coeffs = compVars.slice(0, -1).map((name, j) => ({ name: `ILR_${name}`, b: +beta[1 + j].toFixed(5), se: +(0.1).toFixed(5) }));
